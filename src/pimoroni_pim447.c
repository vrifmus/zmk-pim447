...
enum pim447_mode { PIM447_MODE_MOUSE, PIM447_MODE_SCROLL };

static enum pim447_mode current_mode = PIM447_MODE_MOUSE;

/* Forward declaration of functions */
static void pimoroni_pim447_gpio_callback(const struct device *port,
                                          struct gpio_callback *cb,
                                          gpio_port_pins_t pins);
static int
pimoroni_pim447_enable_interrupt(const struct pimoroni_pim447_config *config,
                                 bool enable);
...
void pim447_enable_sleep(const struct device *dev) {
  ...
  if (i2c_reg_read_byte_dt(&config->i2c, MSK_CTRL_SLEEP, &ctrl_reg_value) !=
      0) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, MSK_CTRL_SLEEP, ctrl_reg_value) !=
      0) {
...
void pim447_toggle_mode(void) {
  current_mode = (current_mode == PIM447_MODE_MOUSE) ? PIM447_MODE_SCROLL
                                                     : PIM447_MODE_MOUSE;
  // Optional: Add logging or LED indication here to show the current mode
  LOG_DBG("PIM447 mode switched to %s",
          (current_mode == PIM447_MODE_MOUSE) ? "MOUSE" : "SCROLL");
}
...
static void pim447_process_movement(struct pimoroni_pim447_data *data,
                                    int delta_x, int delta_y,
                                    uint32_t time_between_interrupts,
                                    int max_speed, int max_time,
                                    float smoothing_factor) {
  float scaling_factor = 1.0f;
  if (time_between_interrupts < max_time) {
    // Exponential scaling calculation
    float exponent = -3.0f * (float)time_between_interrupts /
                     max_time; // Adjust -3.0f for desired curve
  ...
  // Apply smoothing
  data->smoothed_x = (int)(smoothing_factor * scaled_x_movement +
                           (1.0f - smoothing_factor) * previous_x);
  data->smoothed_y = (int)(smoothing_factor * scaled_y_movement +
                           (1.0f - smoothing_factor) * previous_y);
...
static void pimoroni_pim447_work_handler(struct k_work *work) {
  struct pimoroni_pim447_data *data =
      CONTAINER_OF(work, struct pimoroni_pim447_data, irq_work);
  const struct pimoroni_pim447_config *config = data->dev->config;
  ...
  time_between_interrupts =
      data->last_interrupt_time - data->previous_interrupt_time;
  ...
  if (delta_x != 0 || delta_y != 0) {
    if (current_mode == PIM447_MODE_MOUSE) {
      pim447_process_movement(data, delta_x, delta_y, time_between_interrupts,
                              PIM447_MOUSE_MAX_SPEED, PIM447_MOUSE_MAX_TIME,
                              PIM447_MOUSE_SMOOTHING_FACTOR);

      /* Report relative X movement */
      if (delta_x != 0) {
        ret = input_report_rel(data->dev, INPUT_REL_X, data->smoothed_x, true,
                               K_NO_WAIT);
        if (ret) {
      ...
      if (delta_y != 0) {
        ret = input_report_rel(data->dev, INPUT_REL_Y, data->smoothed_y, true,
                               K_NO_WAIT);
        if (ret) {
      ...
      pim447_process_movement(data, delta_x, delta_y, time_between_interrupts,
                              PIM447_SCROLL_MAX_SPEED, PIM447_SCROLL_MAX_TIME,
                              PIM447_SCROLL_SMOOTHING_FACTOR);

      /* Report relative X movement */
      if (delta_x != 0) {
        ret = input_report_rel(data->dev, INPUT_REL_WHEEL, data->smoothed_x,
                               true, K_NO_WAIT);
        if (ret) {
          LOG_ERR("Failed to report delta_x: %d", ret);
        } else {
      ...
      /* Report relative Y movement */
      if (delta_y != 0) {
        ret = input_report_rel(data->dev, INPUT_REL_HWHEEL, data->smoothed_y,
                               true, K_NO_WAIT);
        if (ret) {
          LOG_ERR("Failed to report delta_y: %d", ret);
        } else {
  ...
  if (data->sw_pressed != data->sw_pressed_prev) {
    ret = input_report_key(data->dev, INPUT_BTN_0, data->sw_pressed ? 1 : 0,
                           true, K_NO_WAIT);
    if (ret) {
...
static void pimoroni_pim447_gpio_callback(const struct device *port,
                                          ...
                                          gpio_port_pins_t pins) {
  struct pimoroni_pim447_data *data =
      CONTAINER_OF(cb, struct pimoroni_pim447_data, int_gpio_cb);
...
static void activate_automouse_layer() {
  ...
  k_timer_start(&automouse_layer_timer,
                K_MSEC(CONFIG_ZMK_PIMORONI_PIM447_AUTOMOUSE_TIMEOUT_MS),
                K_NO_WAIT);
}
...
DEVICE_DT_INST_DEFINE(0, pimoroni_pim447_init, NULL, &pimoroni_pim447_data,
                      &pimoroni_pim447_config, POST_KERNEL,
